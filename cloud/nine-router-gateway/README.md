...
- Enforces managed usage accounting for both chat/token consumption and image-generation request counts under plan limits.
...
npm --prefix cloud/nine-router-gateway run test
```
...
- Image generation is treated as a billable unit:
  - `POST /v1/images/generations` is counted as image usage.
  - Free tier requests are rejected.
  - Plus and Pro tiers allow image usage with daily quotas and shared managed windows.
  - Request counts are derived from common payload fields (`n`, `count`, `imageCount`) and normalized to at least `1`.
